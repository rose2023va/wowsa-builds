<?php
/**
 * WOWSA - Streaming - Recent Swim Trips
 * BD Widget Builder widget — inline PHP only.
 *
 * Updated May 9, 2026: data_id changed from 83 (Swim Trip post type, unpublished)
 * to 84 (Swims post type) with wowsa_swim_listing_type = 'Swim Trip' filter.
 *
 * Shows the 6 most recently added Swim Trip-type Swims listings.
 * Card: photo, state_code + country_code location, season months, title, distance pills, View listing button.
 */
...
if (empty($rows)) {
    echo '<p style="color:#999;font-size:14px;">No recent swim trips found.</p>';
    return;
}
...
$metaRows = $w->db->get_results(
    ...
    . " AND `key` IN ('state_code','country_code','distance_bucket','season_months')"
);
$meta = [];
...
<div class="wowsa-stream-grid">
  <?php foreach ($rows as $post):
    ...
    $seasonRaw  = isset($m['season_months'])    ? $m['season_months']    : '';
    $seasonDisplay = '';
    if ($seasonRaw) {
        $slugs = array_filter(array_map('trim', explode(',', $seasonRaw)));
        $seasonDisplay = implode(', ', array_map(
  ...
  <div class="wowsa-stream-card">
    ...
    <div class="wowsa-stream-body">
      ...
      <?php if ($seasonDisplay): ?><p class="wowsa-stream-date"><?= htmlspecialchars($seasonDisplay) ?></p><?php endif; ?>
...
.wowsa-stream-date{font-size:12px;font-weight:600;color:#e76f51;margin:0 0 6px}
...
.wowsa-stream-title a:hover{color:#e76f51}
...
.wowsa-btn-view{display:inline-block;padding:6px 14px;background:#e76f51;color:#fff;border-radius:6px;font-size:12px;font-weight:600;text-decoration:none}
.wowsa-btn-view:hover{background:#c55a40;color:#fff}
</style>
